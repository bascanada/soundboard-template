...
export function downloadVideo(url, cacheDir) {
    ...
    try {
        ...
        execSync(`yt-dlp ${cookieArg} --rm-cache-dir -f "bestvideo+bestaudio/best" --merge-output-format mp4 --no-playlist -o "${outputTemplate}" "${url}"`, { stdio: 'inherit' });
        ...
        try {
            ...
            execSync(`yt-dlp ${cookieArg} -F "${url}"`, { stdio: 'inherit' });
        } catch (e) {
